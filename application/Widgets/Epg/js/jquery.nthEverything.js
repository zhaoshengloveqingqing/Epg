  ...
  $.fn.nthEverything = function() {
    ...
    applyStyles = function(){
        ...
        for (id in parsedStyleMap){
               ...
               if(parsedStyle.element.indexOf("*") < 0 && parsedStyle.element.indexOf("@") < 0 && parsedStyle.element.indexOf("CSS") < 0) {
    ...
    $('link[rel=stylesheet],style').each(function() {
        if ($(this).is('link')) {
            $.get(this.href).success(function(css) { parse(css); });
        } 
        else {
            parse($(this).text());
        }
